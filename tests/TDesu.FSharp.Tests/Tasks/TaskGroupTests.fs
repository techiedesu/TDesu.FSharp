...
type TaskGroupTests() =
    ...
    [<Test>]
    member _.``WaitAll completes all tasks``() =
        task {
            let mutable a = 0
            let mutable b = 0
    ...
    [<Test>]
    member _.``failure cancels other tasks``() =
        task {
    ...
    [<Test>]
    member _.``linked to parent token``() =
        task {
    ...
    [<Test>]
    member _.``multiple errors collected in AggregateException``() =
        task {
            use group = new TaskGroup()
