...
type ChannelWorkerTests() =
    ...
    [<Test>]
    member _.``processes posted items sequentially``() = task {
        use cts = new CancellationTokenSource()
        let processed = Collections.Concurrent.ConcurrentBag<int>()
        let worker = ChannelWorker.start (fun x -> task { processed.Add(x) }) ignore cts.Token
        ...
        do! Task.Delay(200)
        cts.Cancel()
        try do! worker.Completion with :? OperationCanceledException -> ()
    ...
    [<Test>]
    member _.``continues after handler error``() = task {
        use cts = new CancellationTokenSource()
        let processed = Collections.Concurrent.ConcurrentBag<int>()
    ...
    [<Test>]
    member _.``cancellation stops the worker``() = task {
        use cts = new CancellationTokenSource()
        let worker = ChannelWorker.start (fun (_: int) -> Task.CompletedTask) ignore cts.Token
    ...
    [<Test>]
    member _.``post after cancellation does not throw``() = task {
        use cts = new CancellationTokenSource()
        let worker = ChannelWorker.start (fun (_: int) -> Task.CompletedTask) ignore cts.Token
    ...
    [<Test>]
    member _.``PendingCount reflects queued items``() = task {
        use cts = new CancellationTokenSource()
        let gate = new SemaphoreSlim(0)
        let worker = ChannelWorker.start (fun (_: int) -> task { do! gate.WaitAsync() }) ignore cts.Token
    ...
    [<Test>]
    member _.``onError swallowed if it throws``() = task {
        use cts = new CancellationTokenSource()
        let processed = Collections.Concurrent.ConcurrentBag<int>()
        ...
        let worker =
            ChannelWorker.start
                (fun x -> task {
                    if x = 1 then failwith "handler boom"
                    processed.Add(x)
                })
