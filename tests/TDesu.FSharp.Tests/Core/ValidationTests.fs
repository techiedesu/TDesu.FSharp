...
type ValidationTests() =
    ...
    member _.``map preserves Error``() =
        let v: Validation<int, string> = Validation.error "err" |> Validation.map (fun x -> x * 2)
        match v with
    ...
    member _.``bind short-circuits on error``() =
        let v: Validation<int, string> =
            ...
            |> Validation.bind (fun _ -> Validation.error "second")
        match v with
    ...
    member _.``toResult roundtrips``() =
        ...
        isOk 42 okResult
        let errResult: Result<int, string list> = Validation.errors [ "a"; "b" ] |> Validation.toResult
    ...
    member _.``valueOrFail throws on Error``() =
        Assert.Throws<InvalidOperationException>(fun () ->
            Validation.error "bad" |> Validation.valueOrFail |> ignore
        ) |> ignore
    ...
    member _.``validation CE collects errors with and!``() =
        let validateName (s: string) =
            if String.IsNullOrWhiteSpace s then Validation.error "Name required"
