...
type RetryTests() =
    ...
    member _.``withBackoff succeeds on first try``() =
        let result = Retry.withBackoff 3 (TimeSpan.FromMilliseconds 10.) CancellationToken.None (fun () -> task { return 42 }) |> Task.getResult
        equals result 42
    ...
    member _.``withBackoff retries on failure then succeeds``() =
        ...
        let result =
            Retry.withBackoff 3 (TimeSpan.FromMilliseconds 10.) CancellationToken.None (fun () -> task {
                attempt <- attempt + 1
    ...
    member _.``withBackoff throws after max retries``() =
        % Assert.ThrowsAsync<Exception>(fun () ->
            Retry.withBackoff 2 (TimeSpan.FromMilliseconds 1.) CancellationToken.None (fun () -> task {
    ...
    member _.``tryWithBackoff returns Error after max retries``() =
        let result =
            Retry.tryWithBackoff 1 (TimeSpan.FromMilliseconds 1.) CancellationToken.None (fun () -> task {
                return failwith "fail"
            }) |> Task.getResult
        match result with Error _ -> () | Ok _ -> Assert.Fail("Expected Error")

    [<Test>]
    member _.``withDelay retries with fixed delay``() =
        let mutable attempt = 0
        let result =
            Retry.withDelay 2 (TimeSpan.FromMilliseconds 5.) CancellationToken.None (fun () -> task {
                attempt <- attempt + 1
                if attempt < 2 then failwith "not yet"
                return "ok"
            }) |> Task.getResult
        equals result "ok"
...
type TimeoutTests() =
    ...
    member _.``after throws TimeoutException on expiry``() =
        % Assert.ThrowsAsync<TimeoutException>(fun () ->
            Timeout.after (TimeSpan.FromMilliseconds 10.) (fun ct -> task {
    ...
    member _.``afterLinked respects parent cancellation``() =
        ...
        % Assert.ThrowsAsync<OperationCanceledException>(fun () ->
            Timeout.afterLinked (TimeSpan.FromSeconds 10.) parentCts.Token (fun ct -> task {
...
type CircuitBreakerTests() =
    ...
    member _.``closed circuit passes calls through``() =
        let breaker = CircuitBreaker.create { Threshold = 3; Cooldown = TimeSpan.FromSeconds 10. }
        let result = breaker (fun () -> task { return 42 }) |> Task.getResult
        equals result 42

    [<Test>]
    member _.``circuit opens after threshold failures``() =
        let breaker = CircuitBreaker.create { Threshold = 2; Cooldown = TimeSpan.FromSeconds 10. }
        // Fail twice
        for _ in 1..2 do
            try % (breaker (fun () -> task { return failwith "fail" }) |> Task.getResult)
            with _ -> ()
        // Third call should be rejected (circuit open)
        % Assert.ThrowsAsync<InvalidOperationException>(fun () ->
            breaker (fun () -> task { return 42 }) :> Task)
...
type MemoizeTests() =
    ...
    member _.``create caches result``() =
        ...
        let cached = Memoize.create (fun (k: int) -> callCount <- callCount + 1; k * 2)
        equals (cached 5) 10
        equals (cached 5) 10
        equals callCount 1
    ...
    member _.``createAsync caches async result``() =
        let mutable callCount = 0
        let cached = Memoize.createAsync (fun (k: int) -> task {
    ...
    [<Test>]
    member _.``withTtl expires after TTL``() = task {
        let mutable callCount = 0
        let cached = Memoize.withTtl (TimeSpan.FromMilliseconds 50.) (fun (k: int) ->
            callCount <- callCount + 1; k * 2)
        equals (cached 5) 10
        equals (cached 5) 10
        equals callCount 1
        do! Task.Delay(100)
...
type SagaTests() =
    ...
    member _.``run succeeds with all steps``() =
        ...
        match result with Ok v -> equals v 30 | Error _ -> Assert.Fail("Expected Ok")
    ...
    member _.``stepNoCompensate creates step without compensation``() =
        let result =
            Saga.run [
                Saga.stepNoCompensate "only" (fun ctx -> task { return ctx + 1 })
            ] 0 |> Task.getResult
        match result with Ok v -> equals v 1 | Error _ -> Assert.Fail("Expected Ok")
