...
type NonEmptyStringTests() =
    ...
    member _.``createOrFail throws for null``() =
        Assert.Throws<ArgumentNullException>(fun () ->
            NonEmptyString.createOrFail null |> ignore) |> ignore

    [<Test>]
    member _.``createOrFail throws for empty``() =
        Assert.Throws<ArgumentException>(fun () ->
            NonEmptyString.createOrFail "" |> ignore) |> ignore
...
type ApiResponseTests() =
    ...
    member _.``toResult throws when Success=true but Data=None``() =
        let response : ApiResponse.T<int, string> = { Success = true; Data = None; Error = None }
        Assert.Throws<InvalidOperationException>(fun () ->
            ApiResponse.toResult response |> ignore) |> ignore
