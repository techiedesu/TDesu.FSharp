    ...
    let create (config: Config) =
            ...
            lock
                ...
                (fun () ->
                    ...
                    | Closed failures ->
                        ...
                        state.Value <-
                            if n >= config.Threshold then
                                Open(now.Add config.Cooldown)
                            else
                                Closed n
                    | _ -> state.Value <- Open(now.Add config.Cooldown)
        ...
        fun (f: unit -> Task<'T>) ->
            task {
                ...
                | Open _ -> return invalidOp "Circuit is open"
                | HalfOpen ->
                    try
                        let! result = f ()
                        writeState (Closed 0)
