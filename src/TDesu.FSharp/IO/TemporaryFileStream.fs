...
type TemporaryFileStream(?tempFileName, ?doNotDeleteFileAfterDispose) =
    ...
    override this.CanRead with get() = file.CanRead

    override this.CanSeek with get() = file.CanSeek

    override this.CanWrite with get() = file.CanWrite

    override this.Length with get() = file.Length

    override this.Position with get() = file.Position and set value = file.Position <- value

    /// Gets the full path of the temporary file backing this stream.
    member this.FileName with get () = tempFileName

    override this.Flush() =
        file.Flush()

    override this.FlushAsync(ct: CancellationToken) =
        file.FlushAsync(ct)
