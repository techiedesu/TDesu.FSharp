...
type StringBenchmark() =
    ...
    member _.Truncate() : string =
        String.truncate 100 longString

    [<Benchmark(Description = "String.truncate — no-op (short string)")>]
    member _.TruncateNoop() : string =
        String.truncate 100 "short"
    ...
    member _.ManualCount() : int =
        searchString.Split("hello").Length - 1
