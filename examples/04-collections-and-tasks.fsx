...
assertEqual
    "Seq.tryMaxBy picks the element with the greatest projected key"
    (Some "banana")
    (Seq.tryMaxBy String.length [ "a"; "banana"; "kiwi" ])

assertEqual
    "Seq.tryMinBy picks the element with the smallest projected key"
    (Some "a")
    (Seq.tryMinBy String.length [ "a"; "banana"; "kiwi" ])
...
assertEqual
    "ResizeArray.mapi pairs each element with its index"
    [ 0, "a"; 1, "b"; 2, "c" ]
    (ResizeArray.ofList [ "a"; "b"; "c" ]
     |> ResizeArray.mapi (fun i x -> i, x)
     |> ResizeArray.toList)

assertEqual
    "ResizeArray.choose keeps only the Some results, dropping None"
    [ 4; 16 ]
...
assertEqual
    "ResizeArray.rev reverses without mutating the input"
    [ 3; 2; 1 ]
    (ResizeArray.ofList [ 1; 2; 3 ] |> ResizeArray.rev |> ResizeArray.toList)

let evens, odds =
    ResizeArray.ofList [ 1; 2; 3; 4; 5 ]
    |> ResizeArray.partition (fun x -> x % 2 = 0)
...
assertEqual
    "ResizeArray.tryFindIndex finds the first matching index"
    (Some 2)
    (ResizeArray.ofList [ 5; 9; 12; 20 ]
     |> ResizeArray.tryFindIndex (fun x -> x > 10))

assertEqual
    "ResizeArray.tryFindIndex is None when nothing matches"
    None
    (ResizeArray.ofList [ 1; 2 ] |> ResizeArray.tryFindIndex (fun x -> x > 10))

assertTrue
    "ResizeArray.forall is true when every element matches"
    (ResizeArray.ofList [ 2; 4; 6 ] |> ResizeArray.forall (fun x -> x % 2 = 0))

assertTrue
    "ResizeArray.forall is vacuously true for an empty ResizeArray"
    (ResizeArray.create<int> () |> ResizeArray.forall (fun _ -> false))
...
assertEqual
    "TaskGroup.WaitAll only returns once every Run'd task has completed"
    3
    (runBothConcurrently () |> Task.getResult)
...
assertEqual
    "Array.valueTryFind finds the first match"
    (ValueSome 4)
    ([| 1; 4; 6 |] |> Array.valueTryFind (fun x -> x % 2 = 0))

assertEqual
    "Array.valueTryFind reports a miss without allocating an Option"
    ValueNone
    ([| 1; 3 |] |> Array.valueTryFind (fun x -> x % 2 = 0))

assertEqual
    "Array.valueTryFindLast scans from the end"
    (ValueSome 6)
    ([| 1; 4; 6 |] |> Array.valueTryFindLast (fun x -> x % 2 = 0))

assertEqual
    "Array.valueChooseFirst projects while it searches"
    (ValueSome "4")
    ([| 1; 4; 6 |]
     |> Array.valueChooseFirst (fun x -> if x % 2 = 0 then ValueSome(string x) else ValueNone))
